...
// List all payments (optionally by student_id)
router.get('/', (req, res) => {
  const { student_id } = req.query;
  let sql = `SELECT * FROM payments ORDER BY date DESC, id DESC`;
  let params = [];
  if (student_id) {
    sql = `SELECT * FROM payments WHERE student_id = ? ORDER BY date DESC, id DESC`;
    params = [student_id];
  }
...
router.post('/', (req, res) => {
  ...
  const sql = `
    INSERT INTO payments (student_id, amount, date, taxable, method)
    VALUES (?, ?, ?, ?, ?)
  `;
...
// ---------- NEW: Total income between two dates (inclusive) ----------
router.get('/total', (req, res) => {
  ...
  `;
  req.db.get(sql, [start, end], (err, row) => {
    if (err) return res.status(500).json({ error: err.message });
    res.json({ total: row?.total || 0 });
  });
});

// ---------- NEW: Monthly totals for a single student ----------
